    ...
    TraderRegistrationHelper traderRegistrationHelper) : IOnLoad
{
    ...
    public Task OnLoadAsync(CancellationToken cancellationToken)
    {
        ...
        // Pick quest-gated stock first so it cannot also leak into normal stock.
        var restrictedVest = ragmanCandidates
            .Where(x => MatchesAny(x.InternalName, config.RestrictedVestKeywords))
            .OrderByDescending(x => x.PriceRoubles)
            .FirstOrDefault();

        var restrictedHelmet = ragmanCandidates
        ...
        var restrictedWeapon = allWeaponCandidates
        ...
        if (restrictedVest is not null) restrictedSourceKeys.Add(restrictedVest.SourceKey);
        if (restrictedHelmet is not null) restrictedSourceKeys.Add(restrictedHelmet.SourceKey);
        if (restrictedWeapon is not null) restrictedSourceKeys.Add(restrictedWeapon.SourceKey);
        ...
        // Restricted stock. These offers exist in the assort at LL1, but questassort hides them
        // until the matching task is completed.
        OfferResult? quest1Vest = null;
        if (restrictedVest is not null)
        {
            TryCopyOffer(restrictedVest, targetTrader.Assort, copiedSourceOffers, config.RestrictedPriceMarkup,
                1, 1, cloner, out quest1Vest);
        }

        OfferResult? quest2Weapon = null;
        if (restrictedWeapon is not null)
        {
            TryCopyOffer(restrictedWeapon, targetTrader.Assort, copiedSourceOffers, config.RestrictedPriceMarkup,
                1, 1, cloner, out quest2Weapon);
        }

        var quest3Unlocks = new List<OfferResult>();
        if (restrictedHelmet is not null)
        {
            TryCopyOffer(restrictedHelmet, targetTrader.Assort, copiedSourceOffers, config.RestrictedPriceMarkup,
                1, 1, cloner, out var helmetResult);
            if (helmetResult is not null) quest3Unlocks.Add(helmetResult);
        }

        foreach (var candidate in restrictedAmmoCandidates)
        {
            TryCopyOffer(candidate, targetTrader.Assort, copiedSourceOffers, config.RestrictedPriceMarkup,
                config.RestrictedAmmoStock, config.RestrictedAmmoBuyLimit, cloner, out var ammoResult);
            if (ammoResult is not null) quest3Unlocks.Add(ammoResult);
        }

        RegisterQuestChain(targetTrader, templateTable,
            traderRegistrationHelper,
            quest1Vest is null ? [] : [quest1Vest],
            quest2Weapon is null ? [] : [quest2Weapon],
            quest3Unlocks,
            cloner);

        var rootOffers = targetTrader.Assort.Items.Count(x => string.Equals(x.ParentId, "hideout", StringComparison.OrdinalIgnoreCase));
        logger.Success($"[NATO Quartermaster] V1.1 loaded: {rootOffers} curated offers, RUB pricing, dogtag-only buying, and 3 quests.");

        if (quest1Vest is null)
        {
            logger.Warning("[NATO Quartermaster] No matching premium vest was found for quest 1; the quest will still work but has no vest unlock.");
        }
        if (quest2Weapon is null)
        {
            logger.Warning("[NATO Quartermaster] No matching premium weapon preset was found for quest 2; the quest will still work but has no weapon unlock.");
        }
        if (quest3Unlocks.Count == 0)
        {
            logger.Warning("[NATO Quartermaster] No matching restricted helmet/ammunition was found for quest 3.");
        }
    ...
    private static void RegisterQuestChain(
        ...
        ICloner cloner)
    {
        foreach (var unlock in quest1Unlocks)
        {
            targetTrader.QuestAssort["Success"][unlock.RootId] = QuestInventoryCheck;
        }
        foreach (var unlock in quest2Unlocks)
        {
            targetTrader.QuestAssort["Success"][unlock.RootId] = QuestChainOfCustody;
        }
        foreach (var unlock in quest3Unlocks)
        {
            targetTrader.QuestAssort["Success"][unlock.RootId] = QuestRestrictedIssue;
        }
        ...
        string q3DogtagCondition)
    {
        return new Dictionary<string, string>
        {
            ...
            [$"{QuestInventoryCheck} successMessageText"] = "Congratulations. You've successfully completed the work of a moderately competent warehouse clerk. That puts you above average around here. I've opened one of the restricted armor racks.",
            ...
            [$"{QuestChainOfCustody} description"] = "One of my manifests has too many names still marked active. In Tarkov, paperwork tends to lag behind reality. Bring me five PMC dogtags. I don't care which patch they wore. I need proof before I move restricted weapons off the books.",
            ...
            [$"{QuestChainOfCustody} successMessageText"] = "Good. Five names removed from the manifest. That frees one rifle from a shipment that officially never arrived. You can buy it now. Don't make me regret the paperwork.",
            ...
            [$"{QuestRestrictedIssue} description"] = "The last cabinet is controlled stock. Armor, helmets, ammunition that tends to attract questions from people with offices and flags. Bring me ten dogtags from experienced operators, level fifteen or higher. If you're operating at that level, I can justify putting the serious inventory in your hands.",
            [$"{QuestRestrictedIssue} note"] = "Ward's final restricted rack is reserved for proven operators.",
            [$"{QuestRestrictedIssue} startedMessageText"] = "Ten experienced operators. Level fifteen minimum. This isn't a scavenger hunt for fresh tags.",
            [$"{QuestRestrictedIssue} successMessageText"] = "That's enough. The restricted cabinet is open to you. Premium ammunition and the last pieces of protective kit are now on the books — at my prices, naturally.",
